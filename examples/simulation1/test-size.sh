...
rm server.log.*
$MPIEXEC -n $n_server $precommand bash -c "$server_exe_path $total_steps $ensemble_size > server.log.$$ " &

sleep 1

export MELISSA_SERVER_MASTER_NODE="tcp://localhost:4000" 
export MELISSA_SERVER_MASTER_NODE="tcp://jwc00n002.adm00.juwels.fzj.de:4000" 
