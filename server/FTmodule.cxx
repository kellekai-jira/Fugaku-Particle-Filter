...
void FTmodule::store_subset( std::unique_ptr<Field> & field, int state_id, int runner_rank, pthread * new_task )
{
    pthread_t task;
    new_task = NULL;
    int myRank; MPI_Comm_rank(FTI_COMM_WORLD, &myRank);
    if( m_checkpointing ) {
        ...
        if( id_check.find(key) == id_check.end() ) {
            ...
            pthread_create( &task, NULL, pt_fti_add_var, (void*) &id_map[key]);
